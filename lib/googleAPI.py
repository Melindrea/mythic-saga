...
class DriveService:
    ...
    def get_folder_content_list(
        self, folder_id: str, file_fields: list, mime_type: str = None
    ):
    ...
    def copy(self, id: str, parent: str, name: str) -> str:
        copy = (
            self.service.files()
            .copy(fileId=id, body={"parents": [parent], "name": name})
            .execute()
        )
        return copy["id"]
    ...
    def set_permissions(self, id: str, new_permissions: dict):
        return (
            self.service.permissions().create(fileId=id, body=new_permissions).execute()
        )

    def set_editor_permissions(self, e_mail, file_id):
        new_permissions = {"type": "group", "role": "writer", "emailAddress": e_mail}

        return self.set_permissions(id=file_id, new_permissions=new_permissions)

    def set_viewer_permissions(self, file_id):
        new_permissions = {"type": "anyone", "role": "reader"}
...
class SpreadSheetService:
    ...
    def get_template_ids(self, sheet_id) -> tuple[str, str, str]:
        ...
        return (
            get_id_from_drive_url(st_link),
            get_id_from_drive_url(player_link),
            get_id_from_drive_url(character_folder_link),
        )
    ...
    def get_floor_xp(self, id: str):
        floor_xp = (
            self.service.spreadsheets()
            .values()
            .get(spreadsheetId=id, range="'XP log'!J2")
            .execute()
        )

        xp = floor_xp.get("values", [])[0][0]

        return {"cell": "'XP log'!L2", "xp": xp}
    ...
    def batch_update(self, id: str, data: dict) -> dict:
        ...
        return (
            self.service.spreadsheets()
            .values()
            .batchUpdate(spreadsheetId=id, body=batch_body)
            .execute()
        )
...
def get_credentials(
    dir_path: Path, scopes: list[str] = ["https://www.googleapis.com/auth/drive"]
) -> Credentials:
    creds = None
...
def get_service_object(
    service: str, dir_path: Path, scopes: list[str] = None
) -> Resource:
    if scopes is None:
    ...
    try:
        ...
        print(
            err
        )  # HERE BE DRAGONS: I don't like this, but for now it can stay like this
