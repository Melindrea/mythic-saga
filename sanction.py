...
def validate_arguments(args: SheetInformation, override: bool = False) -> list[str]:
    ...
    if not args.email_is_valid():
        errors.append(
            f"The value used for player's e-mail ({args.email}) is not a valid e-mail (probably)."
        )
    ...
    if not args.sanctioned_date_is_valid():
        errors.append(
            f"Invalid data format: {args.given_sanctioned_date}. For use with the -d or --given_sanctioned_date arguments, the usable formats are: 1/31/23, 01/31/23,  1/31/2023, 01/31/2023, or 2023-01-31"
        )
...
def create_connection(
    info: SheetInformation, is_verbose: bool = False, is_dry_run: bool = False
):
...
def file_func(parser: ArgumentParser) -> None:
    ...
    with open(args.path) as csvfile:
        ...
        for row in list_reader:
            ...
            if (
                not sanction_date and args.given_sanctioned_date
            ):  # If the row date fell through, check arg
            ...
            storyteller = (
                row["storyteller"].strip() if row["storyteller"] else args.storyteller
            )
