...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "google_folder_url", help="Link to the folder, gotten via 'Share => Copy link'."
    )
    ...
    parser.add_argument(
        ...
        "--build_url",
        required=False,
        choices=["copy", "view", "edit"],
        help="If the ID should be turned into a copy, view or edit link.",
    )
    parser.add_argument(
        "-md",
        "--markdown",
        action="store_true",
        help="Whether the links should be turned into a markdown list",
    )
    parser.add_argument(
        "-f", "--file", required=False, help="Filename to print the parsed files to."
    )
    ...
    if args.build_url:
        files = [
            (build_url(get_google_type_from_mimetype(n[2]), n[0], args.build_url), n[1])
            for n in files
        ]
